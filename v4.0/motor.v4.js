...
const conf = {
    ...
    totalStatus: function (objAux) {
        ...
        $('fldFiltroAplicado').children[0].innerHTML = `Filtro JSON (${conf.mesAno})`;
    }
}
...
const editarCota = {
    carregarInterface: function (id) {
        aux.ajax({
            urlDoArquivo: 'editar.html', funcaoDeRetorno: (conteudoHtml) => {
...
const html = {
    ...
    exibirCalendarioDaGsv: function (ano) {
        ...
        function _inserirLegenda() {
            ...
            const texto = [
                ...
                'Período de Inscrição para o próximo mês (concorre a 1ª e 2º quinzenas).',
                'Marcação para a 1º quinzena do próximo mês.',
                'Ajuste na inscrição para o próximo mês (concorre apenas à 2º quinzena).',
                'Auditoria interna.'
            ]
    ...
    exibirEstatistica: function () {
        const gerenciadas = dados.parametros();
        const escaladas = Object.assign({}, gerenciadas);
        
        delete gerenciadas.siape;

        console.log(dados.totais('GBM_DESTINO', dados.filtrarEscalas(gerenciadas)));
        console.log(dados.totais('GBM_DESTINO', dados.filtrarEscalas(escaladas)));
    },
